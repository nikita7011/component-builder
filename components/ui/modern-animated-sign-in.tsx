...
const AnimatedForm = memo(function AnimatedForm({
  ...
  const validateForm = (event: FormEvent<HTMLFormElement>) => {
    ...
    fields.forEach((field) => {
      ...
      const element = form.elements.namedItem(field.label) as HTMLInputElement;
    ...
    <section className='max-md:w-full flex flex-col gap-4 w-96 mx-auto'>
      ...
      {googleLogin && (
        <>
          ...
          >
            <button
              className='g-button group/btn bg-white w-full rounded-md border h-10 font-medium outline-hidden hover:cursor-pointer'
              type='button'
              onClick={() => console.log('Google login clicked')}
            >
              <span className='flex items-center justify-center w-full h-full gap-3 text-neutral-800'>
                <Image
          ...
          <BoxReveal boxColor='var(--skeleton)' duration={0.3} width='100%'>
            <section className='flex items-center gap-4'>
              <hr className='flex-1 border-1 border-dashed border-slate-300' />
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          <button
            className='relative group/btn bg-slate-900 w-full text-white rounded-md h-10 font-semibold shadow-lg hover:bg-slate-800 transition-all outline-hidden hover:cursor-pointer'
            type='submit'
          >
            {submitButton} &rarr;
            <BottomGradient />
